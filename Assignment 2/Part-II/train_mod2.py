...
def update_position(hyper_params, population, track): # task 3
    ...
    for i in range(hyper_params['population_size']):
        ...
        if pop[i].curr_car.run(track) > pop[i].worst_car.run(track):
            pop[i].worst_car = pop[i].curr_car
    return pop

#Update velocity
def update_velocity(hyper_params, population, global_best_car, global_worst_car): # task 4
    '''Update velocity of all particles '''
    w, c1, c2, c3, c4 = hyper_params['w'], hyper_params['c1'], hyper_params['c2'], hyper_params['c3'], hyper_params['c4']
    pop = population
    for i in range(hyper_params['population_size']):
        pop[i].vel = w*pop[i].vel + c1*np.random.random()*(pop[i].best_car.weights - pop[i].curr_car.weights) + c2*np.random.random()*(global_best_car.weights - pop[i].curr_car.weights) - c3*np.random.random()*(pop[i].worst_car.weights - pop[i].curr_car.weights) - c4*np.random.random()*(global_worst_car.weights - pop[i].curr_car.weights) 
        pop[i].vel /= (w+c1+c2+c3+c4)
    return pop
...
def PSO(population, track, hyper_params, print_every=10):
    ...
    global_worst_car = min(pop, key=lambda x: x.curr_car.run(track)).curr_car

    for i in range(1,hyper_params['iterations']+1):
        pop = update_velocity(hyper_params, pop, global_best_car, global_worst_car)
        pop = update_position(hyper_params, pop, track)

        best_car = max(pop, key=lambda x: x.curr_car.run(track)).curr_car
        worst_car = min(pop, key=lambda x: x.curr_car.run(track)).curr_car

        if best_car.run(track) > global_best_car.run(track):
            global_best_car = best_car

        if worst_car.run(track) < global_worst_car.run(track):
            global_worst_car = worst_car
...
hyper_params = {
    ...
    'c3': 0.05, # PSO parameter - coefficient of particle's local movement
    'c4': 0.1, # PSO parameter - coefficient of particle's local movement
    'weight_range': (-4,4), # Range in which the weights can vary
